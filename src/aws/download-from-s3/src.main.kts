...
fun S3Client.listObjects(bucket: String, prefix: String, maxKeys: Int? = null): ListObjectsV2Response {
    val request = ListObjectsV2Request.builder().bucket(bucket).prefix(prefix)
    if (maxKeys != null) {
        request.maxKeys(maxKeys)
    }
    return listObjectsV2(request.build())
}
