...
object ReSharperConstants {
    ...
    const val DEFAULT_VERSION = "2026.1" // periodically update this version
}
...
try {
    ...
    val installationType = if (inputs.toolPath.get() != null) InstallationType.GLOBAL_TOOL_PATH else InstallationType.GLOBAL
...
enum class InstallationType {
    GLOBAL {
        override fun composeInstallCommand(inputs: RecipeInputs): List<String> {
            val version = inputs.toolVersion.getRequired()
    ...
    GLOBAL_TOOL_PATH {
        override fun composeInstallCommand(inputs: RecipeInputs): List<String> {
            val version = inputs.toolVersion.getRequired()
            val toolPath = inputs.toolPath.getRequired()
...
data class RecipeInputs(
    ...
    val toolVersion: Input = Input("tool_version", "Tool version", ReSharperConstants.DEFAULT_VERSION),
    val toolPath: Input = Input("tool_path", "Tool path"),
...
class Input(val name: String, val label: String, val default: String? = null) {

    fun get(): String? = System.getenv("input_$name")?.takeIf { it.isNotBlank() }?.let {
        if (default != null && it == "<default>") {
            println("$label: <default> value resolved to $default")
            default
        } else {
            it
        }
